...
public:
    ...
    std::optional<std::vector<uint8_t>> read_lil_block_chain(const uint16_t* chain, size_t chain_length, size_t size) const;
    ...
    bool create_defaults(size_t img_len, uint32_t page_len, SpareType spare_type, uint32_t flash_config, uint32_t fs_offset);
