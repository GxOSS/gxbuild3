...
FlashFileSystem::FlashFileSystem(std::shared_ptr<gxbuild3::utils::FlashBlockDriver> block_driver)
    : m_block_driver(std::move(block_driver)),
      m_corona_data(std::make_shared<XeCoronaFsData>()),
      m_version(0),
      m_start_block_idx(0) {
...
FlashFileSystem::FlashFileSystem(std::shared_ptr<gxbuild3::utils::FlashBlockDriver> block_driver,
    ...
    : m_block_driver(std::move(block_driver)),
      m_corona_data(std::move(corona_data)),
      m_version(0),
      m_start_block_idx(0) {
...
ChainAllocationResult FlashFileSystem::allocate_blocks(uint16_t blocks_needed, uint16_t min_block) {
    if (!m_block_driver) {
    ...
    Log::Error("allocate_blocks: failed to find {} contiguous blocks starting from {}", blocks_needed, min_block);
    return {false, 0};
}
...
size_t FlashFileSystem::set_chain_data(uint16_t start_block, std::span<const uint8_t> data) {
    ...
    const uint32_t blocks_needed = static_cast<uint32_t>(data_size / lil_block_length +
                                                       ((data_size % lil_block_length) > 0 ? 1 : 0));
...
std::optional<std::vector<uint8_t>> FlashFileSystem::get_file_data(const FlashFileSystemEntry& entry) const {
    if (!m_block_driver) {
    ...
    auto result = m_block_driver->read_lil_block_chain(chain->data(), chain_length, entry.length);
    if (!result) {
...
bool FlashFileSystem::set_file_data(FlashFileSystemEntry& entry, std::span<const uint8_t> data) {
    if (entry.block_number == 0) {
    ...
    if (bytes_written != data.size()) {
        Log::Error("set_file_data: failed to write all data (wrote {} of {})", bytes_written, data.size());
        return false;
    }
...
bool FlashFileSystem::load(uint16_t block_idx) {
    ...
    std::vector<uint16_t> new_blockmap(existing_blockmap_size + (blk_map_count * kMaxBlocksPerPage));

    // Copy existing blockmap first (old data at the start)
    if (!m_blockmap.empty()) {
        std::memcpy(new_blockmap.data(), m_blockmap.data(), existing_blockmap_size * sizeof(uint16_t));
    }
    ...
    for (size_t i = 0; i < blk_map_count; i++) {
        ...
        for (size_t y = 0; y < kMaxBlocksPerPage; y++) {
            const size_t dest_idx = y + ((i + existing_blockmap_size / kMaxBlocksPerPage) * kMaxBlocksPerPage);
    ...
    for (size_t y = 0; y < (*fschain).size(); y++) {
        ...
        for (size_t i = 0; i < fs_num; i++) {
            ...
            for (size_t z = 0; z < kMaxEntriesPerPage; z++) {
                ...
                std::memcpy(fs_data + (z * sizeof(FlashFileSystemEntry)), &entry, sizeof(FlashFileSystemEntry));
            }
        }

        // Write the block
        if (!m_block_driver->write_lil_block(current_block, blk_data.data(), lil_block_length)) {
...
const FlashFileSystemEntry* FlashFileSystem::find_entry_internal(std::string_view filename) const {
